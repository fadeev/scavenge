...
const baseMsgCreateScavenge: object = { creator: '', solutionHash: '', description: '', reward: '' }

export const MsgCreateScavenge = {
  encode(message: MsgCreateScavenge, writer: Writer = Writer.create()): Writer {
    if (message.creator !== '') {
      writer.uint32(10).string(message.creator)
    }
    if (message.solutionHash !== '') {
  ...
  decode(input: Reader | Uint8Array, length?: number): MsgCreateScavenge {
    const reader = input instanceof Uint8Array ? new Reader(input) : input
    let end = length === undefined ? reader.len : reader.pos + length
    const message = { ...baseMsgCreateScavenge } as MsgCreateScavenge
    while (reader.pos < end) {
      const tag = reader.uint32()
      switch (tag >>> 3) {
        case 1:
...
const baseMsgCreateScavengeResponse: object = {}

export const MsgCreateScavengeResponse = {
  encode(_: MsgCreateScavengeResponse, writer: Writer = Writer.create()): Writer {
    return writer
  },
  ...
  decode(input: Reader | Uint8Array, length?: number): MsgCreateScavengeResponse {
    const reader = input instanceof Uint8Array ? new Reader(input) : input
    let end = length === undefined ? reader.len : reader.pos + length
    const message = { ...baseMsgCreateScavengeResponse } as MsgCreateScavengeResponse
    while (reader.pos < end) {
      const tag = reader.uint32()
      switch (tag >>> 3) {
        default:
...
export class MsgClientImpl implements Msg {
  ...
  CreateScavenge(request: MsgCreateScavenge): Promise<MsgCreateScavengeResponse> {
    const data = MsgCreateScavenge.encode(request).finish()
    const promise = this.rpc.request('cosmonaut.scavenge.scavenge.Msg', 'CreateScavenge', data)
    return promise.then((data) => MsgCreateScavengeResponse.decode(new Reader(data)))
  }
}
