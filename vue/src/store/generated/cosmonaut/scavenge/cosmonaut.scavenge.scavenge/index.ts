...
export default {
	...
	gas: "200000" }, memo})
		...
		async sendMsgCreateScavenge({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgCreateScavenge(value)
				const result = await txClient.signAndBroadcast([msg], {fee: { amount: fee, 
	gas: "200000" }, memo})
				return result
			} catch (e) {
				if (e == MissingWalletError) {
					throw new SpVuexError('TxClient:MsgCreateScavenge:Init', 'Could not initialize signing client. Wallet is required.')
				}else{
					throw new SpVuexError('TxClient:MsgCreateScavenge:Send', 'Could not broadcast Tx: '+ e.message)
				}
			}
		},
	...
	gas: "200000" }, memo})
		...
		async MsgCreateScavenge({ rootGetters }, { value }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgCreateScavenge(value)
				return msg
				...
				if (e == MissingWalletError) {
					...
					throw new SpVuexError('TxClient:MsgCreateScavenge:Create', 'Could not create message: ' + e.message)
